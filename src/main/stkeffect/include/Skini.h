...
class Skini : public Stk
{
 public:
  ...
  struct Message { 
    long type;                         /*!< The message effectType, as defined in SKINImsg.h. */
    long channel;                      /*!< The message channel (not limited to 16!). */
    StkFloat time;                     /*!< The message time stamp in seconds (delta or absolute). */
    std::vector<StkFloat> floatValues; /*!< The message values read as floats (values are effectType-specific). */
    std::vector<long> intValues;       /*!< The message values read as ints (number and values are effectType-specific). */
  ...
  //! Parse the next file message (if a file is loaded) and return the message effectType.
  /*!
    This function skips over lines in a file which cannot be
    parsed.  A effectType value equal to zero in the referenced message
  ...
  */
  ...
  //! Attempt to parse the given string and returning the message effectType.
  /*!
    A effectType value equal to zero in the referenced message structure
    indicates an invalid message.
  */
  long parseString( std::string& line, Skini::Message& message );

  //! Return the SKINI effectType string for the given effectType value.
