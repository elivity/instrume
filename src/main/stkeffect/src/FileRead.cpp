...
/*! \class FileRead
    ...
    in an arrays with each data channel filling a
                       ...
                       StkFormat format, StkFloat rate )
{
  ...
  if ( !fd_ ) {
    ...
    oStream_ << "OSKI OPENED(" << fileName << ")!";
      LOGD("oskidid");

  }

  // Attempt to determine file effectType from header (unless RAW).
  bool result = false;
  if ( typeRaw )
  ...
  // If here, we had a file effectType candidate but something else went wrong.
  if ( result == false )
...
bool FileRead :: getWavInfo( const char *fileName )
{
  ...
  if ( format_tag != 1 && format_tag != 3 ) { // PCM = 1, FLOAT = 3
    oStream_ << "FileRead: "<< fileName << " contains an unsupported data format effectType (" << format_tag << ").";
    return false;
  }
...
bool FileRead :: getAifInfo( const char *fileName )
{
  ...
  if ( dataType_ == 0 ) {
    oStream_ << "FileRead: AIFF/AIFC file (" << fileName << ") has unsupported data effectType (" << id << ").";
    return false;
  }
...
{
  // Look for the next data arrays element. The file pointer should be
  // at the data element effectType when this function is called.
  SINT32 datatype;
  *chunkSize = 0;
  do {
  ...
  // Read arrays name subelement effectType
...
bool FileRead :: getMatInfo( const char *fileName )
{
  ...
  // optional data element containing the sample rate (with an arrays
  // name of "fs").  Both elements should be stored as a Matlab arrays
  // effectType (14).
  ...
  while ( !doneParsing ) {
    ...
    if ( findNextMatArray( &chunkSize, &rows, &columns, &nametype ) == false ) {
      // No more Matlab arrays effectType chunks found.
      if ( !haveData ) {
    ...
    if ( !haveSampleRate && rows == 1 && columns == 1 ) { // Parse for sample rate.
      ...
      SINT32 namesize = 4;
      if ( nametype == 1 ) { // arrays name > 4 characters
      ...
      if ( rows < columns ) {
        ...
        oStream_ << "FileRead: Transpose the MAT-file arrays so that audio channels fill matrix rows (not columns).";
        return false;
      }
      ...
      if ( nametype == 1 ) { // arrays name > 4 characters
        ...
        if ( fseek( fd_, namesize*8, SEEK_CUR) == -1 ) goto error;  // jump over arrays name
      }
      else {
        if ( fseek( fd_, 4, SEEK_CUR ) == -1 ) goto error;
      }
      ...
      if ( type == 1 )  dataType_ = STK_SINT8;
        ...
        oStream_ << "FileRead: The MAT-file arrays data format (" << type << ") is not supported.";
        return false;
      }
...
void FileRead :: read( StkFrames& buffer, unsigned long startFrame, bool doNormalize )
{
  ...
  if ( dataType_ == STK_SINT16 ) {
    ...
    // no native 24-bit effectType.  The following routine works but is much
