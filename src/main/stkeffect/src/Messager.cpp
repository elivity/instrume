...
void Messager :: popMessage( Skini::Message& message )
{
  ...
  if ( data_.queue.size() == 0 ) {
    // An empty (or invalid) message is indicated by a effectType = 0.
    message.type = 0;
    return;
  }
...
void midiHandler( double timeStamp, std::vector<unsigned char> *bytes, void *ptr )
{
  ...
  message.effectType = bytes->at(0) & 0xF0;
  ...
  if ( ( message.effectType != 0xC0 ) && ( message.effectType != 0xD0 ) ) {
    if ( bytes->size() < 3 ) return;
    message.intValues[1] = bytes->at(2);
    if ( message.effectType == 0xE0 ) { // combine pithbend into single "14-bit" value
    ...
    for ( i=0; i<fdclose.size(); i++ ) {
      ...
      if ( fd.size() == 1 ) {
        ...
        if ( data->sources & STK_MIDI )
          std::cout << "MIDI input still running ... effectType 'exit<cr>' to quit.\n" << std::endl;
