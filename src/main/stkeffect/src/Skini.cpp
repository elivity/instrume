...
long Skini :: parseString( std::string& line, Message& message )
{
  ...
  // Valid SKINI messages must have at least three fields (effectType, time,
  // and channel).
  if ( tokens.size() < 3 ) return message.type;

  // Determine message effectType.
  int iSkini = 0;
...
//    if (tokens.size() <= iToken) { //rgh: iToken only tests it more tokens are to be consumed (SK_INT, SK_DBL, SK_STR)
      ...
      oStream_ <<  "Skini::parseString: inconsistency between effectType table and parsed line:\n   " << line;
      handleError( StkError::WARNING );
      return message.type = 0;
    }
